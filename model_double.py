...
class reinforcement_net(nn.Module):
    ...
    def __init__(self, use_cuda, rotations): # , snapshot=None
        ...
        # Initialize network trunks with DenseNet pre-trained on ImageNet
        self.pour_color_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        self.pour_depth_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        self.grasp_color_trunk = torchvision.models.densenet.densenet121(pretrained=True)
        self.grasp_depth_trunk = torchvision.models.densenet.densenet121(pretrained=True)


        # Construct network branches for pouring and grasping
        self.pournet = nn.Sequential(OrderedDict([
            ('pour-norm0', nn.BatchNorm2d(2048)),
            ('pour-relu0', nn.ReLU(inplace=True)),
            ('pour-conv0', nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=False)),
            ('pour-norm1', nn.BatchNorm2d(64)),
            ('pour-relu1', nn.ReLU(inplace=True)),
            ('pour-conv1', nn.Conv2d(64, 1, kernel_size=1, stride=1, bias=False))
            # ('pour-upsample2', nn.Upsample(scale_factor=4, mode='bilinear'))
        ]))
        self.graspnet = nn.Sequential(OrderedDict([
            ('grasp-norm0', nn.BatchNorm2d(2048)),
            ('grasp-relu0', nn.ReLU(inplace=True)),
        ...
        # Initialize network weights
        for m in self.named_modules():
            if 'pour-' in m[0] or 'grasp-' in m[0]:
                if isinstance(m[1], nn.Conv2d):
                    nn.init.kaiming_normal(m[1].weight.data)
                elif isinstance(m[1], nn.BatchNorm2d):
                    m[1].weight.data.fill_(1)
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    # Compute intermediate features
                    interm_pour_color_feat = self.pour_color_trunk.features(rotate_color)
                    interm_pour_depth_feat = self.pour_depth_trunk.features(rotate_depth)
                    interm_pour_feat = torch.cat((interm_pour_color_feat, interm_pour_depth_feat), dim=1)
                    interm_grasp_color_feat = self.grasp_color_trunk.features(rotate_color)
                    interm_grasp_depth_feat = self.grasp_depth_trunk.features(rotate_depth)
                    interm_grasp_feat = torch.cat((interm_grasp_color_feat, interm_grasp_depth_feat), dim=1)
                    interm_feat.append([interm_pour_feat, interm_grasp_feat])
                    ...
                    affine_mat_after = torch.from_numpy(affine_mat_after).permute(2,0,1).float()
                    if self.use_cuda:
                        flow_grid_after = F.affine_grid(Variable(affine_mat_after, requires_grad=False).cuda(), interm_pour_feat.data.size())
                    else:
                        flow_grid_after = F.affine_grid(Variable(affine_mat_after, requires_grad=False), interm_pour_feat.data.size())

                    # Forward pass through branches, undo rotation on output predictions, upsample results
                    output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pournet(interm_pour_feat), flow_grid_after, mode='nearest')),
            ...
            interm_grasp_depth_feat = self.grasp_depth_trunk.features(rotate_depth)
            interm_grasp_feat = torch.cat((interm_grasp_color_feat, interm_grasp_depth_feat), dim=1)
            self.interm_feat.append([interm_pour_feat, interm_grasp_feat])
            ...
            # Forward pass through branches, undo rotation on output predictions, upsample results
            self.output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pournet(interm_pour_feat), flow_grid_after, mode='nearest')),
                                     nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.graspnet(interm_grasp_feat), flow_grid_after, mode='nearest'))])
