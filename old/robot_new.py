...
with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP','b0RemoteApi', timeout=5) as client:
    ...
    class Robot():
        def __init__(self):
            ...
            self.workspace_limits = np.asarray([[-2, 2], [-6, -2], [-5, -4]]) # note workspace limits
...
# print('Simulation Over.')
    ...
    robot = Robot()
    # input_color_data, input_depth_data = robot.get_input_color_and_depth_data()
    # output_prob = robot.model.forward(input_color_data.cpu(), input_depth_data.cpu())
    # max_idx = torch.argmax(output_prob[0][0][0])
    # _, _, row, col = np.unravel_index(max_idx, shape=output_prob[0][0][0].shape)
