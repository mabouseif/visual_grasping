...
from model_double import reinforcement_net
import logger
...
with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP','b0RemoteApi', timeout=5) as client:
    ...
    class Robot():
        def __init__(self, USE_CUDA=False, LATERAL=False, IS_TESTING=False):
            ...
            _, self.destination_plane_handle = client.simxGetObjectHandle('destination_plane', client.simxServiceCall())
            ...
            _, self.destination_plane_position = client.simxGetObjectPosition(self.destination_plane_handle, -1, client.simxServiceCall())
            ...
            _, self.original_fluid_position = client.simxGetObjectPosition(self.sphere_fluid_handle, -1, client.simxServiceCall())
            ...
            if self.is_testing:
                self.model.load_state_dict(torch.load('/home/mohamed/drive/coppelia_stuff/scripts/logs/mico_cup_1/models/snapshot-000399.grasp.pth'))
            if self.use_cuda:
        ...
        def bowl_full(self):
            ...
            print("THRESH FOR POURING; {}".format( np.sqrt((fluid_position[0] - self.original_bowl_position[0])**2 + (fluid_position[1] - self.original_bowl_position[1])**2 + (fluid_position[2] - self.original_bowl_position[2])**2)))
        ...
        def reset_cup(self):
            ...
            #         ((object_position[2] < self.workspace_limits[2][0]) or (object_position[2] > self.workspace_limits[2][1])):
            relObjHandle = -1
            position = [self.original_cup_position[0], self.original_cup_position[1], self.original_cup_position[2]] # [self.original_cup_position[0] + np.random.uniform(-0.05, 0.05), self.original_cup_position[1] + np.random.uniform(-0.1, 0.1), self.original_cup_position[2]] # [self.original_cup_position[0], self.original_cup_position[1], self.original_cup_position[2]]
        ...
        def reset_fluid(self):
            ...
            position = self.original_fluid_position
        ...
        def close_mico(self):
            ...
            while (j0_position > closing_position) or (j1_position > closing_position): #  and right_force_sensor_feedback[2][2] > -80:
                ...
                print('+'*50)
                if (f1_top[2][2] > 15) and (f2_top[2][2] > 15):
                    return gripper_fully_closed
        ...
        #     # pour_success = False
        #     # if 
        #     # # if (np.pow(position[0] - self.destination_plane_position[0], 2) + np.pow(position[1] - self.destination_plane_position[1], 2)) < thresh:
        #     # if 
        ...
        def trainer_forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
            ...
            for rotate_idx in range(len(output_prob)):
                if rotate_idx == 0:
                    pour_predictions = output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                    ...
                    pour_predictions = np.concatenate((pour_predictions, output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
            ...
            return pour_predictions, grasp_predictions, state_feat
        ...
        def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value): 
            ...
            pour_predictions, grasp_predictions, state_feat = self.trainer_forward(color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=best_pix_ind[0])
            ...
            pour_predictions, grasp_predictions, state_feat = self.trainer_forward(color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=opposite_rotate_idx)
        ...
        def get_label_value(self, primitive_action, pour_success, grasp_success, change_detected, prev_pour_predictions, prev_grasp_predictions, next_color_heightmap, next_depth_heightmap):
            ...
            if primitive_action == 'grasp':
                if grasp_success:
                    ...
                    current_reward = 0.5
            elif primitive_action == 'pour': 
                if pour_success: # maybe intergrate the preceding grasp success in the reward
                    current_reward = 1.0
            
            if pour_success and grasp_success:
                print("Both actions are True")
                exit()
            ...
            if primitive_action == 'grasp':
                if not change_detected and not grasp_success:
                    ...
                    next_pour_predictions, next_grasp_predictions, next_state_feat = self.trainer_forward(next_color_heightmap, next_depth_heightmap, is_volatile=True)
                ...
                if not pour_success:
                    ...
                    next_pour_predictions, next_grasp_predictions, next_state_feat = self.trainer_forward(next_color_heightmap, next_depth_heightmap, is_volatile=True)
                    future_reward = np.max(next_pour_predictions) 
        ...
        def get_action(self, iteration):
            ...
            for i in range(2):
                ...
                pour_predictions, grasp_predictions, state_feat = self.trainer_forward(color_heightmap, valid_depth_heightmap, is_volatile=True)
                ...
                if self.execute_action:
                    ...
                    primitive_action = 'pour' if np.random.randint(0, 2) == 0 else 'grasp'
                    if explore_actions: # Exploitation (do best action) vs exploration (do other action)
                        ...
                        if primitive_action == 'grasp':
                            ...
                            best_pix_ind = [np.random.randint(0, pour_predictions.shape[0]), np.random.randint(0, pour_predictions.shape[1]), np.random.randint(0, pour_predictions.shape[2])]
                            predicted_value = grasp_predictions[best_pix_ind[0]][best_pix_ind[1]][best_pix_ind[2]]
                    else:
                        print('Strategy: exploit (exploration probability: %f)' % (self.explore_prob))
                        # Get pixel location and rotation with highest affordance prediction from heuristic algorithms (rotation, y, x)
                        if primitive_action == 'pour':
                            best_pix_ind = np.unravel_index(np.argmax(pour_predictions), pour_predictions.shape)
                            predicted_value = np.max(pour_predictions)
                    ...
                    # Save executed primitive
                    if primitive_action == 'pour':
                        self.executed_action_log.append([0, best_pix_ind[0], best_pix_ind[1], best_pix_ind[2]]) # 0 - pour
                    elif primitive_action == 'grasp':
                        self.executed_action_log.append([1, best_pix_ind[0], best_pix_ind[1], best_pix_ind[2]]) # 1 - grasp
                    self.logger.write_to_log('executed-action', self.executed_action_log)
                    ...
                    # # Execute Primitive
                    # if self.lateral:
                    #     # grasp_success = self.grasp_lateral(position, best_rotation_angle)
                    #     grasp_success = self.pour(position)
                    # else:
                    #     grasp_success = self.grasp(position, best_rotation_angle)

                    if primitive_action == 'pour':
                        pour_success = self.pour(position)
                    elif primitive_action == 'grasp':
                        grasp_success = self.grasp_lateral(position, best_rotation_angle)
                ...
                if 'prev_color_img' in locals() and not self.is_testing:
                    ...
                    if self.experience_replay:
                        ...
                        if sample_ind.size > 0:
                            ...
                            with torch.no_grad():
                                sample_pour_predictions, sample_grasp_predictions, sample_state_feat = self.trainer_forward(sample_color_heightmap, sample_depth_heightmap, is_volatile=True)
                    ...
                    # if not prev_grasp_success:
                    #     self.reset_cup()
                    # if primitive_action == 'grasp' and prev_primitive_action == 'grasp':
                    #     self.reset_cup()
                    if prev_primitive_action == 'grasp':
                        if not prev_grasp_success:
                            self.reset_cup()
                            client.simxSleep(0.2)
                            self.reset_fluid()
                            self.reset_fluid()
                    if prev_primitive_action == 'pour':
                        self.reset_cup()
                        client.simxSleep(0.2)
                        self.reset_fluid()
                        self.reset_fluid()
                ...
                prev_pour_predictions = pour_predictions.copy()
                prev_grasp_success = grasp_success if 'grasp_success' in locals() else False
                prev_pour_success = pour_success if 'pour_success' in locals() else False
...
######################################################################################################
    ...
    robot = Robot(USE_CUDA=False, LATERAL=True, IS_TESTING=False)
