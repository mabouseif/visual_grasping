...
with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP','b0RemoteApi', timeout=5) as client:
    ...
    class Robot():
        def __init__(self, USE_CUDA=False, LATERAL=False, IS_TESTING=False):
            ...
            _, self.j0=client.simxGetObjectHandle("JacoHand_fingers12_motor1", client.simxServiceCall())
            _, self.j1=client.simxGetObjectHandle("JacoHand_fingers12_motor2", client.simxServiceCall())
            _, self.j2=client.simxGetObjectHandle("JacoHand_finger3_motor1", client.simxServiceCall())
            _, self.j3=client.simxGetObjectHandle("JacoHand_finger3_motor2", client.simxServiceCall())
            _, self.f1_bottom = client.simxGetObjectHandle("JacoHand_forceSens1_finger1", client.simxServiceCall())
            _, self.f1_top = client.simxGetObjectHandle("JacoHand_forceSens2_finger1", client.simxServiceCall())
            _, self.f2_bottom = client.simxGetObjectHandle("JacoHand_forceSens1_finger2", client.simxServiceCall())
            _, self.f2_top = client.simxGetObjectHandle("JacoHand_forceSens2_finger2", client.simxServiceCall())
            _, self.f3_bottom = client.simxGetObjectHandle("JacoHand_forceSens1_finger3", client.simxServiceCall())
            _, self.f3_top = client.simxGetObjectHandle("JacoHand_forceSens2_finger3", client.simxServiceCall())
            ...
            self.workspace_limits = np.asarray([[-0.7, -0.3], [-0.2, 0.2], [0.225, 0.4]])
        ...
        def reset_cup(self):
            ...
            #         ((object_position[2] < self.workspace_limits[2][0]) or (object_position[2] > self.workspace_limits[2][1])):
            relObjHandle = -1
            position = [self.original_cup_position[0], self.original_cup_position[1] + np.random.uniform(-0.3, 0.3), self.original_cup_position[2]]
        ...
        def close_jaco(self):
            ...
            client.simxSetJointForce(self.j2, motor_force, client.simxServiceCall())
            client.simxSetJointForce(self.j3, motor_force, client.simxServiceCall())
            client.simxSetJointTargetVelocity(self.j0, -.7*closingVel, client.simxServiceCall())
            client.simxSetJointTargetVelocity(self.j1, -.7*closingVel, client.simxServiceCall())
            client.simxSetJointTargetVelocity(self.j2, -closingVel, client.simxServiceCall())
            client.simxSetJointTargetVelocity(self.j3, -closingVel, client.simxServiceCall())
            ...
            _, j2_position = client.simxGetJointPosition(self.j2, client.simxServiceCall())
            _, j3_position = client.simxGetJointPosition(self.j3, client.simxServiceCall())
            ...
            f3_bottom= client.simxReadForceSensor(self.f3_bottom, client.simxServiceCall())
            f4_top= client.simxReadForceSensor(self.f3_top, client.simxServiceCall())
            gripper_fully_closed = False
            closing_position = -0.046# -4.462e-02 # starting -2.742e-06
            while (j0_position > closing_position):#  or (j1_position > closing_position) or (j2_position > closing_position) or (j3_position > closing_position):#  and right_force_sensor_feedback[2][2] > -80:
                ...
                if (new_j0_position >= j0_position):#  or (new_j1_position >= j1_position) or (new_j2_position >= j2_position) or (new_j3_position >= j3_position): # If no change, AND not reached closing position, then gripper is not fully closed
                    return gripper_fully_closed
        ...
        def grasp_lateral(self, position, best_rotation_angle):
            ...
            gripper_full_closed = self.close_jaco()
...
######################################################################################################
    ...
    robot = Robot(USE_CUDA=True, LATERAL=True, IS_TESTING=False)
