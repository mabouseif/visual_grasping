    ...
    with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP','b0RemoteApi', timeout=5) as client:
        ...
        class Robot():
            def __init__(self, USE_CUDA=False, LATERAL=False, IS_TESTING=False):
                ...
                if self.is_testing:
                    self.model.load_state_dict(torch.load('/home/mohamed/drive/coppelia_stuff/scripts/logs/mico_cup_1/models/snapshot-000449.grasp.pth'))
                if self.use_cuda:
            ...
            def reset_cup(self):
                ...
                #         ((object_position[2] < self.workspace_limits[2][0]) or (object_position[2] > self.workspace_limits[2][1])):
                relObjHandle = -1
                position = [self.original_cup_position[0], self.original_cup_position[0] + np.random.uniform(-0.1, 0.1), self.original_cup_position[2]]
                orientation = [np.random.uniform(0, 2*np.pi), np.random.uniform(0, 2*np.pi), 0]
            ...
            def grasp_lateral(self, position, best_rotation_angle):
                ...
                location_above_grasp_target[2] = location_above_grasp_target[2] + 0.2
