    ...
    with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP','b0RemoteApi', timeout=5) as client:
        ...
        class Robot():
            def __init__(self, USE_CUDA=False, LATERAL=False, IS_TESTING=False):
                ...
                if self.is_testing:
                    self.model.load_state_dict(torch.load('/home/mohamed/drive/coppelia_stuff/scripts/logs/multi_1/models/snapshot-001499.grasp.pth'))
                if self.use_cuda:
            ...
            def grasp(self, position, best_rotation_angle):
                ...
                height_z = 0.01 if self.lateral else 0.04
                position[2] = max(position[2] - height_z, self.workspace_limits[2][0] + 0.02)
                ...
                # Compute gripper orientation and rotation increments
                if self.lateral:
                    tool_rotation_angle_lateral = -np.pi
                    rotation_step_lateral = 0.05 if (tool_rotation_angle_lateral - gripper_orientation[2] > 0) else -0.05
                    num_rotation_steps_lateral = int(np.floor((tool_rotation_angle_lateral - gripper_orientation[2])/rotation_step_lateral))
            ...
            def grasp_lateral(self, position, best_rotation_angle):
                ...
                for step_iter in range(max(num_move_steps, num_rotation_steps)):
                    client.simxSetObjectPosition(self.target_right_handle,-1,(UR5_target_position[0] + move_step[0]*min(step_iter,num_move_steps), UR5_target_position[1] + move_step[1]*min(step_iter,num_move_steps), UR5_target_position[2] + move_step[2]*min(step_iter,num_move_steps)),client.simxServiceCall())
                    client.simxSetObjectOrientation(self.target_right_handle, -1, (np.pi/2, np.pi/2, gripper_orientation[2] + rotation_step*min(step_iter,num_rotation_steps)), client.simxServiceCall())
                client.simxSetObjectPosition(self.target_right_handle,-1,(tool_position[0],tool_position[1],tool_position[2]),client.simxServiceCall())
                client.simxSetObjectOrientation(self.target_right_handle, -1, (np.pi/2, np.pi/2, tool_rotation_angle), client.simxServiceCall())
            ...
            def get_prediction_vis(self, predictions, color_heightmap, best_pix_ind):
                ...
                for canvas_row in range(int(num_rotations/4)):
                    ...
                    for canvas_col in range(4):
                        ...
                        if best:
                            best_img = prediction_vis
                            # best_img = cv2.circle(prediction_vis, (int(best_pix_ind[2]), int(best_pix_ind[1])), 7, (255,255,255), -1) # (0,0,255)
                            best = False
                ...
                for i in range(2):
                    ...
                    if self.execute_action:
                        ...
                        if i == 0 and grasp_success:
                            grasp_pred_vis, best = self.get_prediction_vis(grasp_predictions, color_heightmap, best_pix_ind)
                    ...
                    if 'prev_color_img' in locals(): #  and not self.is_testing:
                        ...
                        self.explore_prob = max(0.5 * np.power(0.9995, iteration),0.1) if self.explore_rate_decay else 0.5
